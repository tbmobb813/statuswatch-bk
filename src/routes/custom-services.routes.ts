...
function getMaxServicesForUser(user: { stripePriceId?: string | null }): number {
  if (!user.stripePriceId) return MAX_CUSTOM_SERVICES_FREE;

  // Check subscription tier (this is simplified - adjust based on your pricing)
  if (user.stripePriceId.includes('enterprise')) return MAX_CUSTOM_SERVICES_ENTERPRISE;
  if (user.stripePriceId.includes('pro')) return MAX_CUSTOM_SERVICES_PRO;
...
router.post('/', authMiddleware, validate(createCustomServiceSchema), async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
    const data = req.body;
...
router.patch('/:id', authMiddleware, validate(updateCustomServiceSchema), async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const userId = req.userId!;
    const data = req.body;
...
router.get('/limits/info', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const userId = req.userId!;
