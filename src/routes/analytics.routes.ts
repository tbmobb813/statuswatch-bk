...
import { authMiddleware, AuthRequest, optionalAuth } from '../middleware/auth.middleware';
...
router.get('/mttr', optionalAuth, async (req: AuthRequest, res) => {
  try {
...
router.get('/mttd', optionalAuth, async (req: AuthRequest, res) => {
  try {
    const serviceId = req.query.serviceId as string | undefined;
    const days = parseInt(req.query.days as string) || 30;
...
router.get('/reliability', optionalAuth, async (req: AuthRequest, res) => {
  try {
    const serviceId = req.query.serviceId as string | undefined;
    const days = parseInt(req.query.days as string) || 30;
...
// Get SLA compliance
router.get('/sla', optionalAuth, async (req: AuthRequest, res) => {
  try {
    const serviceId = req.query.serviceId as string | undefined;
...
router.get('/comparison', optionalAuth, async (req: AuthRequest, res) => {
  try {
    const days = parseInt(req.query.days as string) || 30;
    ...
    const comparison = Array.from(serviceMap.values()).sort(
