...
class NotificationService {
    ...
    async sendNotification(payload) {
        try {
            ...
            if (alertPreference.discordWebhook) {
                await this.sendDiscordNotification(alertPreference.discordWebhook, payload);
            }
            // Send Slack webhook if configured
            if (alertPreference.slackWebhook) {
                await this.sendSlackNotification(alertPreference.slackWebhook, payload);
            }
    ...
    async notifyStatusChange(serviceId, oldStatus, newStatus, message) {
        try {
            ...
            for (const monitored of service.monitoredServices) {
                ...
                if (pref && shouldNotify(pref)) {
                    await this.sendNotification({
                        ...
                        message: `${service.name} status changed from ${oldStatus} to ${newStatus}${message ? `: ${message}` : ''}`,
                        type: 'status_change',
