...
class AnalyticsService {
     ...
     */
    async calculateMTTR(serviceId, days = 30) {
     ...
     */
    async calculateMTTD(serviceId, days = 30) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        const where = {
            startedAt: { gte: startDate },
     ...
     */
    async calculateReliabilityScore(serviceId, days = 30) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
     ...
     */
    async getIncidentTrends(serviceId, days = 30) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        const where = {
            startedAt: { gte: startDate },
     ...
     */
    async getSummaryAnalytics(days = 30) {
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        const [totalIncidents, activeIncidents, totalServices, mttrData, mttdData, reliabilityData,] = await Promise.all([
            // Total incidents
